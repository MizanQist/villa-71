...
`villa-71-brochure.pdf` is a sixteen-sheet A4-landscape version of the site without the 360° tour: cover, at a glance, two scheme sheets, grounds with the site plan, three interior sheets (eighteen views each), four plan sheets, the climate section at 09:00, 12:00 and 15:00, the specification, contact and a back sheet. It is generated by `tools/make_pdf.py` (Python 3 with Pillow and Playwright + Chromium), which reuses the site's markup for the description, plans, specification, strategies and credits, re-encodes the images into `tools/print/img/` and prints `tools/print/print.html` at 297 × 210 mm. Re-run it after editing copy or images. The PDF is not linked from the site; share it directly.
