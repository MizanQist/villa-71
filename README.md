...
Ten pages, one file: `index.html` carries the markup, styles and interactions; `assets/` holds the visualisations (exteriors at the top level at full size for the lightbox, with the 1400 px copies the page itself shows in `assets/m/`; interiors in `assets/i/` with `assets/t/` thumbnails), the four floor plans and the site plan. Fonts load from Google Fonts (Cormorant, Manrope).
...
- `Villa 71/Guzape Plans For Brochure All.pdf` — the June 2026 drawing set (A_71_23, Arc. Sadiq Abu): site plan, basement, ground, first and second floor plans, two elevations and a section. The floor plans on page 07 come from it (`plan-basement/ground/first/second.png`, `elev-a.jpg`, `elev-b.jpg`, `section.jpg`), and page 08's sun study is a section through the approach balcony (53 m², 3.7 m deep, the Suite 6 balcony 2.7 m above it and the roof with its slats), drawn in the Governor's Lodge manner with the profile angle simplified to the sun's altitude.
...
- Copy lives in `index.html`. The four schemes and their captions are in the `OPTS` object in the script; the plan tabs in `PLAN`; the tour's room order and names in `ROOMS` (files `assets/vr/NN.jpg` in that order).
...
- Page 08 (climate and ground) uses the sun's altitude only, so it needs no north point; the section geometry (floor heights, projections) is set in the constants at the top of its script block.
