...
`villa-71-brochure.pdf` is a thirty-nine-page A4-landscape edition without the 360° tour, one subject to a page for A4 printing: cover, at a glance, a scheme overview and then each exterior view on its own page (the chalet last), the pool, the two terraces, the courtyard and the site plan each on a page, the interiors in the website's order at no more than four views a page (the main rooms on pages of their own, the guest chalet last), the plans in the order basement, ground, first, second and then the chalet on its own, the climate section at 09:00, 12:00 and 15:00, the specification, contact and a back page. `tools/make_pdf.py` builds `tools/print/print.html` from the site's markup with `tools/print/print.css`, re-encodes the images into `tools/print/img/` and prints at 297 × 210 mm with Playwright; content keeps 14 mm margins so nothing important is lost on a printer that cannot print to the edge. Re-run it after editing copy or images. The PDF is not linked from the site; share it directly.
